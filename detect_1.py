...
Usage - sources:
    $ python detect_1.py --weights yolov5s.pt --source 0                               # webcam
                                                     img.jpg                         # image
                                                     vid.mp4                         # video
                                                     path/                           # directory
                                                     'path/*.jpg'                    # glob
...
Usage - formats:
    $ python detect_1.py --weights yolov5s.pt                 # PyTorch
...
def run(
        ...
        data=ROOT / 'data/datasettest.yaml',  # dataset.yaml path
        ...
        coords_red=None,
        coord=''):
    if coords_red is None:
        coords_red = []
    if coords_grey is None:
        coords_grey = []
    source = str(source)
    ...
    for path, im, im0s, vid_cap, s in dataset:
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if "car" in label:
                            coord_1_grey = xyxy
                            for i in coord_1_grey:
                                coord_2_grey = re.sub('\D', '', str(i))
                                #print(coord_2_grey)
                                #print(type(coord_2_grey))
                                coord_stripped_grey.append(coord_2_grey)
                            ...
                            # [tensor(51.), tensor(180.), tensor(977.), tensor(589.)]
                            # [tensor(30.), tensor(300.), tensor(1200.), tensor(700.)]
                            #test = [30, 300, 1200, 700]
                        try:
                            if len(coords_red) > 0 and len(coords_grey) > 0:
                                diff = numpy.subtract(int(coords_grey[-1]),
                                                      int(coords_red[-1]))  # [-1] for last element
        ...
        # Print resolution, objects detected, time (inference-only)
        LOGGER.info(f"{s}{'' if len(det) else '(no detections), '}{dt[1].dt * 1E3:.1f}ms")
        s_list = s.split(" ")
        #print("the data", s_list)
        #Checks list of objects detected for given one
        #object_detected = input("Object:")
        #object_detected = 'chair'
        #if any(s.startswith(object_detected) for s in s_list) == True:
        #    print("found object")
        #else:
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/datasettest.yaml', help='(optional) dataset.yaml path')
